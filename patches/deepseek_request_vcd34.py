...
def _inject_stub_thinking_for_tool_use(messages: Any) -> Any:
    """Inject stub thinking blocks before tool_use in assistant messages that lack them.

    DeepSeek v4-pro and v4-flash always emit thinking blocks. On multi-turn requests,
    DeepSeek requires those thinking blocks to be passed back in the history. When the
    upstream client (claude code) strips thinking from its context, this function injects
    an empty stub thinking block so DeepSeek's requirement is satisfied.
    """
    if not isinstance(messages, list):
        return messages
    result: list[Any] = []
    for message in messages:
        ...
        if not isinstance(content, list):
            result.append(message)
            continue
        # Check if there's a tool_use block without a preceding thinking block
        has_tool_use = any(
        ...
        if has_tool_use and not has_thinking:
            # Inject a stub thinking block at the start of the content
            new_content: list[Any] = [{"type": "thinking", "thinking": ""}] + list(content)
            new_msg = dict(message)
            new_msg["content"] = new_content
            result.append(new_msg)
            logger.debug(
                "DEEPSEEK_REQUEST: injected stub thinking for tool follow-up assistant message"
            )
        else:
            result.append(message)
    return result
...
def build_request_body(request_data: Any, *, thinking_enabled: bool) -> dict:
    ...
    # VCD-34 r5: deepseek-v4-pro/-flash always emit thinking blocks. DeepSeek requires
    # them passed back on every turn. Keep thinking enabled unconditionally so that:
    # (a) thinking is not stripped from history, (b) stub thinking blocks are injected
    # for assistant messages that claude code stripped thinking from.
    # The upstream unsafe_tool_followup bypass is removed — it caused 400 errors.
    effective_thinking_enabled = thinking_enabled
    if thinking_enabled:
        if has_tool_history:
            logger.debug(
                "DEEPSEEK_REQUEST: keeping thinking for tool follow-up (stub injection) "
                "model={} msgs={} tools={}",
                data.get("model"),
    ...
    if "messages" in data:
        data["messages"] = _strip_reasoning_content_when_native(
            _normalize_tool_result_content(
                _inject_stub_thinking_for_tool_use(
